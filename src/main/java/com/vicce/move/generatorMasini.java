...
public class generatorMasini {
    ...
    //     for(int i=0;i<n;i++){
    //         float viteza = vitezaMaxima[(int) new Random().nextFloat(vitezaMaxima.length)];
    //         float pretul = pret[(int) new Random().nextFloat(pret.length)];
    ...
    //         double Greutate = greutate[(int) new Random().nextDouble(greutate.length)];
    ...
    //         double km = kmReali[(int) new Random().nextDouble(kmReali.length)];
